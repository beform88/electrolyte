...
from .conc_predict import ConcPredict_Model
...
class ElectrolyteModel(nn.Module):
    ...
    def __init_models_(self, ):
        ...
        self.conc_predict = ConcPredict_Model(input_dim=self.hidden_dim, hidden_dim=self.hidden_dim)
    ...
    def forward(self, property, solvent, solute_anion, solute_cation):
        ...
        self.features = torch.cat([solvent_out, solute_anion_out, solute_cation_out], dim = 0)

        self.sub = self.features
        self.env = torch.cat([self.features,property_embed.reshape([1,-1])]).unsqueeze(0).repeat(self.sub.shape[0], 1, 1)

        self.conc = self.conc_predict(self.sub, self.env)

        return self.conc.reshape([-1])
